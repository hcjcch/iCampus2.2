...
public class MessagePost extends Activity {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.job_publish_over:
			map = getMap();
			if (!CheckInformation.checkQQ(qq)) {
				Toast.makeText(MessagePost.this, "请输入正确的QQ!",
						Toast.LENGTH_SHORT).show();
				return false;
			}
			if (!CheckInformation.chaeckPhone(mobile)) {
				Toast.makeText(MessagePost.this, "请输入正确的电话号码!",
						Toast.LENGTH_SHORT).show();
				return false;
			}
			if (!CheckInformation.checkMail(email)) {
				Toast.makeText(MessagePost.this, "请输入正确的邮箱!",
						Toast.LENGTH_SHORT).show();
				return false;
			}
